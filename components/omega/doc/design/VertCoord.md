...
Similar to MPAS-Ocean's support for tilted height ($z^\star$) coordinates, Omega will support a general ($p^\star$) vertical coordinate.
...
This involves adding a vertically-weighted bottom pressure perturbation to the a reference (initial) thickness.
...
### Desired: General Arbitrary Lagrangian Eulerian (ALE) coodrinate support
...
where $\mathbf{u}_h$ is the horizontal velocity, $f$ is the Coriolis parameter, $v = \frac{1}{\rho}$ is the specific volume, $\rho = \rho(\Theta,S,p)$ is the density, $p$ is the hydrostatic pressure, $\Phi$ is the geopotential, and $\boldsymbol{\mathcal{F}}$ are the dissipative terms.
...
$$ z_{i,k+1/2} = z_i^{floor} + \sum_{k=k^\prime}^{N} v_{i,k^\prime} h_{i,k^\prime}, $$

where $z_i^{floor}$ is the (positive-up) bottom elevation.
The $z$ location of cell centers is given by: 

$$ z_{i,k} = z_i^{floor} + \frac{1}{2} v_{i,k} h_{i,k} + \sum_{k^\prime= k+1}^{N} v_{i,k^\prime} h_{i,k^\prime}. $$
...
where the the pressure alteration, $h_k^{p}$, is given by:
...
class VertCoord {
    ...
    private:
        ...
        Array2DI4 MinLevelCell;
        Array2DI4 MaxLevelCell;
        Array2DI4 MinLevelEdgeTop;
        Array2DI4 MaxLevelEdgeTop;
        Array2DI4 MinLevelEdgeBot;     
        Array2DI4 MaxLevelEdgeBot;
        Array2DI4 MinLevelVertexTop;
        Array2DI4 MaxLevelVertexTop;     
        Array2DI4 MinLevelVertexBot;
        Array2DI4 MaxLevelVertexBot;
...
```
The user will select vertical coordinate options at runtime in the input yaml file under the vertical coordinate section:
```yaml
    VerticalCoordinate:
       NVertLevels: 60
```
...
The public `computeZHeight` will sum the mass thicknesses times $v$ from the bottom later up, starting with the bottom elevation
...
### Test: 
Unit tests will be used to test each of the computations (computePressure, computeZHeight, computeGeopotential, computePStarThickness) for a given mass thickness array. Comparison will be made to a ``truth'' vertical mesh.
