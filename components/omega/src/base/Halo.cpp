...
Halo::ExchList::ExchList(
   ...
   // First dimension of List is the halo width
   I4 HaloWidth = List.size();

   // Set member vector sizes to halo width
   NList.resize(HaloWidth);
   Offsets.resize(HaloWidth);
...
Halo::Neighbor::Neighbor(
   ...
   const std::vector<std::vector<I4>> &SendVert, // list of vertices to send
   ...
   const std::vector<std::vector<I4>> &RecvVert, // list of vertices to receive
   ...
   SendLists[2] = ExchList(SendVert);

   RecvLists[0] = ExchList(RecvCell);
   RecvLists[1] = ExchList(RecvEdge);
   RecvLists[2] = ExchList(RecvVert);
...
Halo::Halo(const MachEnv *InEnv, const Decomp *InDecomp) {
   ...
   for (int INghbr = 0; INghbr < NNghbr; ++INghbr) {
      Neighbors.push_back(Neighbor(SendCellLists[INghbr],
                                   SendEdgeLists[INghbr],
                                   SendVrtxLists[INghbr],
                                   RecvCellLists[INghbr],
                                   RecvEdgeLists[INghbr],
                                   RecvVrtxLists[INghbr],
                                   NeighborList[INghbr]));
...
int Halo::generateExchangeLists(
   ...
   // the neighboring task IDs and save them in NeighborList and save the number
   ...
   if(First) {
      for (int ILayer=0; ILayer<HaloWidth; ++ILayer) {
         for (int Idx = HaloBnds[ILayer]; Idx < HaloBnds[ILayer + 1]; ++Idx) {
            ...
            auto it = std::find(NeighborList.begin(), NeighborList.end(), NewVal);
            if (it == NeighborList.end()) {
               std::vector<I4> TmpVec(HaloWidth, 0);
               ++TmpVec[ILayer];
      ...
      for (int ILayer = 0; ILayer < HaloWidth; ++ILayer) {
         for (int Idx = HaloBnds[ILayer]; Idx < HaloBnds[ILayer + 1]; ++Idx) {
            I4 NewVal = LocPtr(Idx, 0);
            I4 INghbr = srchVector(NeighborList, NewVal);
            ++NumNghbrHalo[INghbr][ILayer];
         }
      }
   }
   ...
   for (int INghbr = 0; INghbr < NNghbr; ++INghbr) {
      HaloIdx[INghbr].resize(HaloWidth);
      RecvLists[INghbr].resize(HaloWidth);
      for (int ILayer = 0; ILayer < HaloWidth; ++ILayer) {
   ...
   // Save the needed halo info from Decomp and save in RecvLists and HaloIdx.
   // RecvLists will contain the local indices of halo elements that belong to
   // neeighbors, sorted by neighbor and halo layer, these are then ready to
   ...
   for (int ILayer=0; ILayer<HaloWidth; ++ILayer) {
      ...
      for (int Idx=HaloBnds[ILayer]; Idx < HaloBnds[ILayer + 1]; ++Idx) {
         I4 INghbr = srchVector(NeighborList, LocPtr(Idx, 0));
   ...
   for(int INghbr = 0; INghbr<NNghbr; ++INghbr) {
      SendLists[INghbr].resize(HaloWidth);
      I4 IOffset = 0;
      for(int ILayer = 0; ILayer < HaloWidth; ++ILayer) {
...
int Halo::exchangeVectorInt(
   ...
   for(int INghbr = 0; INghbr < NNghbr; ++INghbr) {
      ...
      if (SendErr[INghbr] != 0) {
         LOG_CRITICAL("MPI error {} on task {} send to task {}",
   ...
   for(int INghbr = 0; INghbr < NNghbr; ++INghbr) {
      ...
      RecvErr[INghbr] = MPI_Recv(&RecvVec[INghbr][0], DimLen, MPI_INT,
                        NeighborList[INghbr], 0, MyComm, MPI_STATUS_IGNORE);
      if (RecvErr[INghbr] != 0) {
         LOG_CRITICAL("MPI error {} on task {} receive from task {}",
...
int Halo::startSends() {
   ...
   for (int INghbr = 0; INghbr < NNghbr; ++INghbr) {
      ...
      IErr[INghbr] = MPI_Isend(&MyNeighbor->SendBuffer[0], BufferSize,
                     MPI_RealKind, MyNeighbor->TaskID, 0, MyComm,
                     &MyNeighbor->SReq);
      if (IErr[INghbr] != 0) {
...
int Halo::packBuffer(const ArrayHost3DI4 Array) {
   ...
   for (int K = 0; K < NK; ++K) {
      for (int ILayer = 0; ILayer < HaloWidth; ++ILayer) {
         for (int IExch = 0; IExch < MyList->NList[ILayer]; ++IExch) {
            for (int J = 0; J < NJ; ++J) {
               I4 IBuff = (K * MyList->NTot + MyList->Offsets[ILayer] + IExch)
                          * NJ + J;
            ...
            for (int IExch = 0; IExch < MyList->NList[ILayer]; ++IExch) {
               for (int J = 0; J < NJ; ++J) {
                  I4 IBuff = ((L * NK + K) * MyList->NTot +
                     MyList->Offsets[ILayer] + IExch) * NJ + J;
                  MyNeighbor->SendBuffer[IBuff] = reinterpret_cast<Real&>(
